  ...
  try {
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        const deleted = await prisma.sMSTemplate.deleteMany({
          where: {
            id: { in: ids },
          },
        })
      ...
      case 'export':
        ...
        const rows = smsTemplates.map((template) => [
          template.id,
          template.name,
          template.type,
          template.message,
        ...
        return new NextResponse(csv, {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="sms-templates-export-${format(new Date(), 'yyyy-MM-dd')}.csv"`,
          },
        })
