...
import { Save, Loader2, User, Mail, Phone, Lock, Upload, X, Shield, CheckCircle, FileText, Clock } from 'lucide-react'
...
export function OwnerProfilePage() {
  ...
  const [aiReportPrefs, setAiReportPrefs] = useState<AIReportPreferences>({
    enabled: false,
    frequency: 'none',
    preferredTime: '08:00',
    lastSent: null,
  ...
  const fetchAIReportPreferences = async () => {
    try {
      const res = await fetch('/api/owner/ai-reports', {
        credentials: 'include',
      })
  ...
  const handleSaveAIReportPreferences = async () => {
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ error: 'Failed to update AI report preferences' }))
        throw new Error(errorData.error || 'Failed to update AI report preferences')
      }
      ...
      toast.success('AI report preferences updated successfully')
    } catch (error: any) {
      toast.error(error.message || 'Failed to update AI report preferences')
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Receive automated AI-powered reports about your properties via email
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Receive automated reports with insights about your properties
              </p>
            </div>
            <Switch
          ...
          {aiReportPrefs.enabled && (
            <div className="space-y-4 pt-4 border-t">
              <div className="space-y-2">
                <Label htmlFor="ai-report-frequency">Report Frequency</Label>
                <Select
                ...
                <p className="text-xs text-gray-500">
                  {aiReportPrefs.frequency === 'daily'
                    ? 'Receive a report every day'
                    : aiReportPrefs.frequency === 'weekly'
                    ? 'Receive a report once per week'
                    : 'Reports are disabled'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="ai-report-time" className="flex items-center gap-2">
                ...
                />
                <p className="text-xs text-gray-500">
                  Time of day to receive reports (24-hour format)
                </p>
              </div>
                ...
                <div className="pt-4 border-t space-y-2">
                  {aiReportPrefs.lastSent && (
                    <div className="text-sm">
                      ...
                      <span className="font-medium">
                        {new Date(aiReportPrefs.lastSent).toLocaleString()}
                      </span>
                    </div>
                  )}
                  {aiReportPrefs.nextSend && (
                    <div className="text-sm">
                      ...
                      <span className="font-medium">
                        {new Date(aiReportPrefs.nextSend).toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
