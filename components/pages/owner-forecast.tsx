...
interface OwnerForecastProps {
  ownerId?: string
}

export function OwnerForecast({ ownerId }: OwnerForecastProps) {
  ...
  const generateForecast = async () => {
    ...
    try {
      const res = await fetch(`/api/ai/forecast?ownerId=${ownerId}`)
      if (!res.ok) {
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Generate AI-powered revenue and occupancy forecasts for your entire portfolio
            </p>
            <Button onClick={generateForecast} disabled={loading || !ownerId}>
              {loading ? (
                <>
